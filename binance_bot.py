...
def main():
    ...
    # Aggressive Hour-Boundary Trigger (v3.15)
    # Compares current UTC hour string (e.g. '09') with last reported hour
    now_utc = time.gmtime()
    curr_hour_str = time.strftime('%H', now_utc)
    last_hour_str = metadata.get('last_reported_hour', '')
    
    should_send = curr_hour_str != last_hour_str
    
    if should_send:
        metadata['last_reported_hour'] = curr_hour_str
        metadata['last_report_time'] = time.time()
        logger.info(f"Report Triggered: Hour transition detected ({last_hour_str} -> {curr_hour_str}).")
    else:
        mins_until_next = 60 - now_utc.tm_min
        logger.info(f"Monitoring: UTC Hour {curr_hour_str} in progress. Next report in ~{mins_until_next}m.")
